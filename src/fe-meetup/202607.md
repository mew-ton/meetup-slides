...
**エコシステム全体が 7 に乗れるわけではない**

- `vue-tsc` (Volar) は TS の **コンパイラ API に割り込んで** `.vue` の型を解決する仕組み
- TS 7.0 の Go 実装には **その API の安定版がまだ無い** → vue-tsc などは未対応
- Vue / Svelte / Astro / MDX を使うワークフローは **当面 TS 6 系に留まる** ことに
- 安定 API は **TS 7.1 (2026 年 10 月頃見込み)** で提供され, 解消される予定

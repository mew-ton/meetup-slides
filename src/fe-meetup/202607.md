...
- SQL インジェクションと REST API の経路混同, 2 つの脆弱性を連鎖させると
  **ログイン不要でサーバー上の任意コード実行** (RCE) に到達
- **プラグインなしの素の WordPress (6.9〜7.0.1) で成立** する点が異例
  (CVSS は最大 9.8 / Critical 評価)
- 7/17 に **緊急パッチ (6.9.5 / 7.0.2)** が公開され, 強制自動更新も発動
- 直後から **実際の攻撃と, 公開された実証コード (PoC)** が確認されている

<!-- _footer: "出典: [WordPressの脆弱性「wp2shell」 (GIGAZINE)](https://gigazine.net/news/20260721-wordpress-wp2shell-exploit/) / [WP2Shell WordPress Vulnerabilities Exploited in the Wild (SecurityWeek)](https://www.securityweek.com/wp2shell-wordpress-vulnerabilities-exploited-in-the-wild/)" -->
